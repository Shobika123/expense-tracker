...
const Expenses = (props) => {
  ...
  const getData = useCallback(async () => {
    try {
      const res = await fetch(
        `https://expense-tracker-2-4007e-default-rtdb.firebaseio.com/expense${email}.json`
      );
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (isEdit === true) {
        res = await fetch(
          `https://expense-tracker-2-4007e-default-rtdb.firebaseio.com/expense${email}/${editId}.json`,
          {
            method: "put",
  ...
  const deleteHandler = async (id) => {
    const res = await fetch(
      `https://expense-tracker-2-4007e-default-rtdb.firebaseio.com/expense${email}/${id}.json`,
      {
        method: "delete",
      }
    );
